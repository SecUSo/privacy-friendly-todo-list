...
class MigrationV2ToV3: MigrationBase(2, 3) {
    override fun doMigrate(db: SupportSQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS `${TABLE_NAME}` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `listId` INTEGER, `sortOrder` INTEGER NOT NULL, `name` TEXT NOT NULL, `description` TEXT NOT NULL, `priority` INTEGER NOT NULL, `deadline` INTEGER, `recurrencePattern` INTEGER NOT NULL, `recurrenceInterval` INTEGER NOT NULL, `reminderTime` INTEGER, `progress` INTEGER NOT NULL, `creationTime` INTEGER NOT NULL, `doneTime` INTEGER, `isInRecycleBin` INTEGER NOT NULL, FOREIGN KEY(`listId`) REFERENCES `todoLists`(`id`) ON UPDATE CASCADE ON DELETE SET NULL )")
        ...
        db.execSQL("INSERT INTO $TABLE_NAME (creationTime, recurrencePattern, recurrenceInterval, id, listId, sortOrder, name, description, priority, deadline, reminderTime, progress, doneTime, isInRecycleBin)" +
                " SELECT _id, _id, _id, _id, todo_list_id, _id, name, description, priority, deadline, deadline_warning_time, progress, done, in_trash FROM todo_task")
        ...
        // Set default value in new column recurrenceInterval.
        db.execSQL("UPDATE $TABLE_NAME SET recurrenceInterval = 1")
        ...
        cursor.use {
            ...
            while (wasMoved) {
                ...
                val sortOrder2 = sortOrders[listId] ?: 0
        ...
        cursor.use {
            ...
            while (wasMoved) {
                ...
                val sortOrder3 = sortOrders[taskId] ?: 0
